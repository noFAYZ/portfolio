...
export function Blog({ onOpenPost }: { onOpenPost: (slug: string) => void }) {
  ...
  const cardClassName =
    "group border-border bg-card hover:border-border focus-visible:ring-ring relative block w-full min-w-0 flex-1 -rotate-1 text-left shadow-sm border p-2 transition-[transform,border-color] duration-100 ease-out hover:rotate-0 focus-visible:ring-2 focus-visible:outline-none"
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <span className="text-muted-foreground group-hover:text-foreground mt-1.5 flex items-center gap-1 text-xs transition-colors duration-150">
            read it
            <Icon
              icon={external ? "lucide:arrow-up-right" : "lucide:arrow-right"}
  ...
  return (
    <section id="writing" className="mx-auto flex w-full max-w-5xl flex-col   px-4 py-6 sm:px-6">
      <SectionStamp index={1}>Writing</SectionStamp>
      <motion.div {...rise()} className="flex items-center gap-1">
        {sorted.length > 1 && (
          <Button
            size="icon-sm"
            variant="ghost"
